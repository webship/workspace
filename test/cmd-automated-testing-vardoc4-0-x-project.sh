...
ddev composer create-project vardot/vardoc-project:${site_version} . --no-interaction;
...
sed -i "s,127.0.0.1:4445/wd/hub,${automated_testing_wd_host},g" ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/vardoc/behat.yml ;
sed -i "s,http://vardoc.test,${base_url},g" ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/vardoc/behat.yml ;

webmaster_testing_user_target="webmaster: { email: '${account_mail}', password: '${account_pass}' }" ;

sed -i "s/webmaster: { email: 'webmaster@vardot.com', password: 'dD.123123ddd' }/${webmaster_testing_user_target}/g" ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/vardoc/behat.yml ;

if ! $headless ; then
  sed -i "s,- \"--headless\",#- \"--headless\",g" ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/vardoc/behat.yml ;
fi
...
ddev drush site:install vardoc --yes --site-name="${doc_name} ${PROJECT_NAME}"  --account-name="${account_name}"  --account-pass="${account_pass}"  --account-mail="${account_mail}"  --db-url="mysql://db:db@db/db" --locale="en"  vardoc_extra_components.vardoc_demo=true -vvv;
...
# Add testing users.
cd ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/vardoc/scripts
...
echo " cd ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/vardoc";
echo "-----------------------------------------";
...
if $run_automated_testing ; then
  cd ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/vardoc;
