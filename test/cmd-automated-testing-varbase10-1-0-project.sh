...
ddev config --project-type=drupal --docroot=docroot --project-name=${PROJECT_NAME} --auto ;
ddev start ;

ddev composer create-project vardot/varbase-project:${site_version} . --no-interaction;
...
cp ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/sites/default/default.settings.php ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/sites/default/settings.php ;
...
];" >> ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/sites/default/settings.php ;

mkdir -p ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/config/sync ;
echo "\$settings['config_sync_directory'] = '${config_sync_directory}';" >> ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/sites/default/settings.php ;

build_time=$( date '+%Y-%m-%d %H-%M-%S' );
echo "// Built time: ${build_time}" >> ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/sites/default/settings.php ;

sed -i "s,127.0.0.1:4445/wd/hub,${automated_testing_wd_host},g" ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/contrib/varbase/behat.yml ;
sed -i "s,http://varbase.test,${base_url},g" ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/contrib/varbase/behat.yml ;

webmaster_testing_user_target="webmaster: { email: '${account_mail}', password: '${account_pass}' }" ;

sed -i "s/webmaster: { email: 'webmaster@vardot.com', password: 'dD.123123ddd' }/${webmaster_testing_user_target}/g" ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/contrib/varbase/behat.yml ;

if ! $headless ; then
  sed -i "s,- \"--headless\",#- \"--headless\",g" ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/contrib/varbase/behat.yml ;
fi

sed -i "s,/home/rajab/workspace/test/varbase/docroot,${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot,g" ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/contrib/varbase/behat.yml ;

# Change directory to the docroot.
cd ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot;

# Install Varbase with Drush.
ddev drush site:install varbase --yes --site-name="${doc_name} ${PROJECT_NAME}"  --account-name="${account_name}"  --account-pass="${account_pass}"  --account-mail="${account_mail}"  --db-url="mysql://db:db@db/db" --locale="en" varbase_multilingual_configuration.enable_multilingual=true varbase_extra_components.vmi=true varbase_extra_components.varbase_heroslider=true varbase_extra_components.varbase_carousels=true varbase_extra_components.varbase_search=true varbase_extra_components.varbase_blog=true varbase_extra_components.varbase_auth=true varbase_extra_components.editoria11y=true install_configure_form.enable_update_status_emails=NULL -vvv;
...
# Add testing users.
cd ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/contrib/varbase/scripts
ddev exec bash add-testing-users.sh

# Send a notification.
echo "${doc_name} ${PROJECT_NAME} has been installed!!!!";
echo "-----------------------------------------";
echo " Change direcotry to the project:"
echo " cd ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/contrib/varbase";
echo "-----------------------------------------";
...
if $run_automated_testing ; then
  cd ${WEBSHIP_WORKSPACE_ROOT}/${doc_name}/${PROJECT_NAME}/docroot/profiles/contrib/varbase;
