...
if [ -d "${WORKSPACE_ROOT}/${doc_name}/alpha" ]; then
  (cd "${WORKSPACE_ROOT}/${doc_name}/alpha" && ddev delete -y -O 2>/dev/null) ;
  rm -rf "${WORKSPACE_ROOT}/${doc_name}/alpha" ;
fi

mkdir -p "${WORKSPACE_ROOT}/${doc_name}/alpha" ;
cd "${WORKSPACE_ROOT}/${doc_name}/alpha" ;
