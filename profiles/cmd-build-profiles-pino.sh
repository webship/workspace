...
if [ -d "${WORKSPACE_ROOT}/${doc_name}/pino" ]; then
  (cd "${WORKSPACE_ROOT}/${doc_name}/pino" && ddev delete -y -O 2>/dev/null) ;
  rm -rf "${WORKSPACE_ROOT}/${doc_name}/pino" ;
fi

mkdir -p "${WORKSPACE_ROOT}/${doc_name}/pino" ;
cd "${WORKSPACE_ROOT}/${doc_name}/pino" ;
