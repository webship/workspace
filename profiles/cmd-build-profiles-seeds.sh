...
if [ -d "${WORKSPACE_ROOT}/${doc_name}/seeds" ]; then
  (cd "${WORKSPACE_ROOT}/${doc_name}/seeds" && ddev delete -y -O 2>/dev/null) ;
  rm -rf "${WORKSPACE_ROOT}/${doc_name}/seeds" ;
fi

mkdir -p "${WORKSPACE_ROOT}/${doc_name}/seeds" ;
cd "${WORKSPACE_ROOT}/${doc_name}/seeds" ;
