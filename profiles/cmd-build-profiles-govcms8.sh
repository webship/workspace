...
if [ -d "${WORKSPACE_ROOT}/${doc_name}/dev" ]; then
  (cd "${WORKSPACE_ROOT}/${doc_name}/dev" && ddev delete -y -O 2>/dev/null) ;
  rm -rf "${WORKSPACE_ROOT}/${doc_name}/dev" ;
fi

mkdir -p "${WORKSPACE_ROOT}/${doc_name}/dev" ;
cd "${WORKSPACE_ROOT}/${doc_name}/dev" ;
