...
# Propose one agent back to the shared repository — webship/ai-agents by default, or whatever
...
ARGPARSE_DESCRIPTION="Propose one agent to ${AI_ITEMS_REPO}"
source ${WORKSPACE_SCRIPTS}/args/arg-propose-item.sh || exit 1 ;

shift $#;

items_propose "agents" "${NAME}" "${SUMMARY}" "$([ "${CONFIRM}" == 'yes' ] && echo yes || echo no)" ;
